...
type _$add2<
  ...
  OUTPUT_DEC extends DigitList.DigitList = 
    HAS_CARRY extends true
      ? SUM extends [SUM[0], ...infer Rest] 
        ? [...Rest, ...A_DEC_REM] 
        : never
      : [...SUM, ...A_DEC_REM],
  OUTPUT extends DecimalDigitList.DecimalDigitList = [OUTPUT_INT, ...DigitList._$trimRight<OUTPUT_DEC>]
> = OUTPUT;
...
export type _$add<
  ...
  A_LENGTH extends Number.Number = NaturalNumber._$decrement<A["length"]>,
  B_LENGTH extends Number.Number = NaturalNumber._$decrement<B["length"]>,
  IS_A_LONGER extends Boolean = Number._$compare<A_LENGTH, B_LENGTH> extends 1 | 0 ? true : false,
  SUM = IS_A_LONGER extends true 
    ? _$add2<A, B, B_LENGTH> 
    : _$add2<B, A, A_LENGTH>,
  RESULT = SUM extends [] ? [0, Digit.Zero] : SUM
> = RESULT
